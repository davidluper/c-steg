...
void print_help() {
	std::cout << "this is the c-steg command line utility" << std::endl
			<< std::endl;

	std::cout << "\tto encode a lossless jpeg image with text" << std::endl;
	std::cout << "\t\t--encode-lossless" << std::endl;
	std::cout << "\t\t--input-file\t\t<required string (path to input image)>"
			<< std::endl;
	std::cout << "\t\t--output-file\t\t<required string (path to output image)>"
			<< std::endl;
	std::cout << "\t\t--text\t\t\t<required string>" << std::endl;
	std::cout << "\t\t--quality\t\t<optional int defaults to 71>" << std::endl;
	std::cout << "\t\t--bits-to-steal\t\t<optional int defaults to 2>"
			<< std::endl;
	std::cout << "" << std::endl;
	...
	std::cout << "\t\t--quality\t\t<optional int defaults to 71>" << std::endl;
	std::cout
			<< "\t\t--compressions\t\t<optional int defaults to 5 (number of times "
			<< std::endl
			<< "\t\t\t\t\t to uncompress/recompress the image before "
			<< std::endl << "\t\t\t\t\t encoding the payload)>" << std::endl;
	std::cout << "" << std::endl;
	...
	std::cout << "\tto get the number of usable coefficients in a file"
			<< std::endl;
	...
	std::cout << "\t\t--input-file-1\t\t<required string (path to an image)>"
			<< std::endl;
	std::cout << "\t\t--input-file-2\t\t<required string (path to an image)>"
			<< std::endl;
	std::cout << "" << std::endl;
	std::cout << "" << std::endl;
}
...
int main(int argc, const char* argv[]) {
	...
	if (argc <= 1) {
		std::cout << "***********************" << std::endl;
		std::cout
				<< "you need to pass in some command line flags, here is the help documentation"
				<< std::endl;
	...
	try {
		for (int i = 1; i < argc; ++i) {
			if (strcmp("--help", argv[i]) == 0) {
				...
				std::cout << "invalid command line arg, use --help to see what commands are valid" << std::endl;
				exit(1);
			}
		}
	} catch (...) {
		std::cout << "error parsing command line parameters, use --help to see what commands are valid" << std::endl;
		exit(1);
	}
